...
class MeteorCommand {
	...
	public execute(): Thenable<string> {
		return new Promise((resolve, reject) => {
			...
			const envArgsString = this.environmentArgs && ConfigHelper.mapEnvArgsToString(this.environmentArgs);

			const executeOutputString = envArgsString
				? `MeteorHelper: executing '${command}' with environment variables '${envArgsString}'\n`
				: `MeteorHelper: executing '${command}'\n`;
			this.outputChannel.append(this, executeOutputString);
...
export class MeteorCommandHelper {
	private static outputChannelRun: OutputChannelWrapper = new OutputChannelWrapper(
		vscode.window.createOutputChannel('Meteor Run')
	);
	private static outputChannelTest: OutputChannelWrapper = new OutputChannelWrapper(
		vscode.window.createOutputChannel('Meteor Test')
	);
	private static outputChannelMisc: OutputChannelWrapper = new OutputChannelWrapper(
		vscode.window.createOutputChannel('Meteor')
	);
	...
	public static registerCommandWithUserInput(
		commandName: string,
		isOptionalInput: boolean = false,
		...args: string[]
	): vscode.Disposable {
		return vscode.commands.registerCommand(commandName, () => {
	...
	public static stopCommand(commandName: string, ...args: string[]): vscode.Disposable {
		return vscode.commands.registerCommand(commandName, () => {
			...
			vscode.window
				.showQuickPick(runningCommands, { placeHolder: 'Select a running meteor process to stop..' })
	...
	private static execMeteorCommand(
		args: string[],
		force = false,
		visible = false,
		type: CommandType,
		...
		if (!command) {
			...
			if (!ConfigHelper.isMeteorProjectFolder(meteorAppPath)) {
				vscode.window.showErrorMessage(
					`MeteorHelper: ${meteorAppPath} is not a Meteor project directory. Check your workspace configuration.`
				);
				return;
			}
			...
			vscode.window.showInformationMessage(
				`MeteorHelper: Meteor ${command.args[0]} already running (PID: ${command.pid}), stop Meteor first..`
			);
			return;
		} else {
			vscode.window.showInformationMessage(
				`MeteorHelper: already running meteor ${command.args[0]}, wait for command to finish..`
			);
			return;
		}
	...
	private static statusBarStop(commandType: CommandType): void {
		switch (commandType) {
			case CommandType.Run:
			case CommandType.Debug:
				this.statusBar.setMeteorRunStatusBar(
					RunMode.Stopped,
					'Click to start meteor run',
					'meteorhelper.meteorRun'
				);
				break;
			case CommandType.Test:
				this.statusBar.setMeteorTestStatusBar(
					TestMode.Stopped,
					'Click to start meteor test',
					'meteorhelper.meteorTest'
				);
				break;
	...
	private static setMeteorTestConfig(commandName: string, args: string[]): void {
		...
		if (meteorVersion < 1.3) {
			vscode.window.showErrorMessage(
				`MeteorHelper: meteor test requires at least meteor 1.3. Meteor version being used in this project is version ${meteorVersion}.`
			);
			return;
		}
		...
		if (!testConfig || !testConfig.driverPackage) {
			vscode.window
				.showInformationMessage(
					'MeteorHelper: no valid Meteor testing configuration in VSCode Workspace Configuration',
		...
		if (testConfig.settings) {
			if (!shell.test('-f', testConfig.settings)) {
				vscode.window
					.showErrorMessage(
						`MeteorHelper: Settings file ${testConfig.settings} specified in testing configuration not found!`,
						'Open Workspace Settings'
					)
					.then(() => vscode.commands.executeCommand('workbench.action.openWorkspaceSettings'));
				return;
		...
		const envArgs = ConfigHelper.mapKeyValueConfigToEnvArgs(testConfig.envArgs);
	...
	private static meteorRunDebug(commandName: string, args: string[]): void {
		...
		const keyValueConfigItems = ConfigHelper.getEnvArgsConfig();
		const envArgs = ConfigHelper.mapKeyValueConfigToEnvArgs(keyValueConfigItems);
